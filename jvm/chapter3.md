...
![](image/gc-recovery-algorithm-2.png)
...
#### 常用参数
1. -XX:+UseG1GC 开启G1
1. -XX:G1HeapRegionSize=n,region的大小，1-32M，2048个
1. -XX:MaxGCPauseMillis=200 最大停顿时间
1. -XX:G1NewSizePercent   -XX:G1MaxNewSizePercent
1. -XX:G1ReservePercent=10 保留防止to space溢出（）
1. -XX:ParallelGCThreads=n SWT线程数（停止应用程序）
1. -XX:ConcGCThreads=n 并发线程数=1/4*并行

#### 最佳实践
1. 年轻代大小：避免使用-Xmn、-XX:NewRatio等显示设置Young区大小，会覆盖暂停时间目标（常用参数3）
1. 暂停时间目标：暂停时间不要太严苛，其吞吐量目标是90%的应用程序时间和10%的垃圾回收时间，太严苛会直接影响到吞吐量
...
1. -Xmx:最大堆大小，默认物理内存的1/4
1. -Xmn:新生代大小
1. -XX:NewSize:设置新生代初始大小
1. -XX:NewRatio:默认2表示新生代占年老代的1/2，占整个堆内存的1/3。
1. -XX:SurvivorRatio:默认8表示一个survivor区占用1/8的Eden内存，即1/10的新生代内存。
1. -XX:MetaspaceSize:设置元空间大小
1. -XX:MaxMetaspaceSize:设置元空间最大允许大小，默认不受限制，JVM Metaspace会进行动态扩展。
### 垃圾回收统计信息
1. -XX:+PrintGC
1. -XX:+PrintGCDetails
...
1. -XX:+UseParallelOldGC:老年代使用并行回收收集器
1. -XX:+UseParNewGC:在新生代使用并行收集器
1. -XX:+UseParalledlOldGC:设置并行老年代收集器
1. -XX:+UseConcMarkSweepGC:设置CMS并发收集器
1. -XX:+UseG1GC:设置G1收集器
1. -XX:ParallelGCThreads:设置用于垃圾回收的线程数
### 并行收集器设置
1. -XX:ParallelGCThreads:设置并行收集器收集时使用的CPU数。并行收集线程数。
1. -XX:MaxGCPauseMillis:设置并行收集最大暂停时间
1. -XX:GCTimeRatio:设置垃圾回收时间占程序运行时间的百分比。公式为1/(1+n)
### CMS收集器设置
1. -XX:+UseConcMarkSweepGC:设置CMS并发收集器
